...
async def get_curriculum_and_journal(
    family=Depends(require_parent),  # ✅ pass dependency correctly
    ...
    for student in students:
        subjects = session.exec(
            select(Subjects).where(Subjects.student_id == student.student_id)
        ).all()
        lessons = session.exec(
            select(Lessons).where(Lessons.student_id == student.student_id)
        ).all()

        journal = {}
        for lesson in lessons:
            week = f"Week {lesson.curriculum_day // 5 + 1 if lesson.curriculum_day else 1}"
